...
.DESCRIPTION
    Output:
      hubspot-object-reference.md    — the document
...
.PARAMETER OutputPath
    ...
    Defaults to hubspot-object-reference.md inside the hubspot output folder.
#>
...
if (-not $OutputPath) {
    $OutputPath = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot '../../output/hubspot-object-reference.md'))
}
...
$doc.AppendLine("# HubSpot — Object Reference")              | Out-Null
...
foreach ($name in $objectNames) {
    ...
    if (-not (Test-Path $csvFile)) {
        Write-Warning "$name.csv not found, skipping"
        continue
    }

    $doc.AppendLine("")            | Out-Null
    $doc.AppendLine("### $name")   | Out-Null
    $doc.AppendLine("")            | Out-Null
    $doc.AppendLine((ConvertTo-MdTable $csvFile)) | Out-Null
    Write-Host "Table: $name" -ForegroundColor DarkGray
}
